...
class Tree(types.DiscreteVariable):
    ...
    def grow(self, cumulative_sum_list, rng=RNG()):
        action = "grow"
        self.lastAction = action
        '''
        grow tree by just simply creating the individual nodes. each node
        ...
        # if u_sample>0.9999:
        #     u = rng.uniform()
        #     feature, interval = self.find_interval(u, cumulative_sum_list)
        # else:
        feature = rng.randomInt(0, len(self.X_train[0])-1)
        # generating a random threshold
        threshold = rng.randomInt(0, len(self.X_train)-1)
    ...
    def change(self,cumulative_sum_list, rng=RNG()):
        action = "change"
        self.lastAction = action
        '''
        ...
        #use the mic metric to change with 80% possibilities
        # if u_sample>0.9999:
        #     u = rng.uniform()
        #     new_feature, interval = self.find_interval(u, cumulative_sum_list)
        # else:
