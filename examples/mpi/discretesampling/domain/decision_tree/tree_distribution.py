...
class TreeProposal(DiscreteVariableProposal):
    def __init__(self, tree, rng=RNG()):
        ...
        self.moves_prob = [0.1, 0.1, 0.4, 0.4]  # good for Poisson and heart l = 12, and diabetes l = 10
        self.rng = rng
    ...
    def sample(self, cumulative_sum_list,  num_nodes=20):
        ...
        if random_number < moves_probabilities[0]:
            ...
            # change
            newTree = newTree.change(cumulative_sum_list, rng=self.rng)

        else:
            # grow
            newTree = newTree.grow(cumulative_sum_list, rng=self.rng)
