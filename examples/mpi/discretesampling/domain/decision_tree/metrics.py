...
class stats():
    ...
    def print_top_k_performing_trees(smcAccuracy_diagnostics, treeSMCSamples, k, weights):
        ...
        return top_k_performing_trees, av_acc
...
def trees_similarity(tree1, tree2):
    ...
    points_list = [0.5]
    if tree1_depth >= tree2_depth:#create the points list based on the biggest tree
        big_tree = tree1
        small_tree = tree2
        for i in (n+1 for n in range(tree1_depth)):
            points_list.append(points_list[i-1]/2)
    else:
        big_tree = tree2
        small_tree = tree1
        for i in (n+1 for n in range(tree2_depth)):
            points_list.append(points_list[i-1]/2)
